...
5. **Severity-Farbe trägt Bedeutung — sonst neutral.** Zahlen in KPI-Karten sind neutral-schwarz (`#0a0a0a`), nicht rot. Rot/Orange/Gelb/Grün nur dort, wo der Wert *eine Stufe ausdrückt* (Alerts, Status-Badges).
...
### 6.6 Status-Badge (nicht-Severity)
Für Zustände wie `OBSERVED`, `VERIFIED`, `New`, `Open`. Neutrale Variante: Mono 11px/600, `color #737373`, `bg #f3f3f3`, `border 1px #e6e6e6`. „OBSERVED" darf Amber (`#854d0e`/`#fefaeb`) tragen, „VERIFIED" Grün (`#157f3b`/`#f0faf3`).
...
### Agent Chains
- Liste anklickbarer Chain-Zeilen: Chain-ID (Mono) · Agent · Status-Badge (`OBSERVED` amber, §6.6) · rechts Zeitstempel (Mono). Ausgewählte Zeile leicht hervorgehoben (`bg #f3f3f3`).
- **Detail-Banner** im selben „minimal"-Geist (Status-Eyebrow + Titel „Untitled Session — Alerts raised (2)" + Meta-Zeile in Mono).
